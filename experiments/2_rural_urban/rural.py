...
class Agent:
    __slots__ = ['id', 'x', 'y', 'sugar', 'metabolism', 'vision', 'broadcast_radius', 'messages', 'destination']
    ...
    def move(self, environment):
        ...
        if self.messages:
            ...
            if best_message and best_message['sugar_amount'] > visible_sugar:
    ...
    def broadcast_message(self, environment, timestep, tree):
        if not tree:
        ...
        # Find neighbors within broadcast_radius using cKDTree
        indices = tree.query_ball_point([self.x, self.y], self.broadcast_radius)

        for idx in indices:
            if idx == self.id:
                continue  # Skip self
            neighbor_agent = environment.agents[idx]
            neighbor_agent.messages.append(message)
...
class SugarscapeEnvironment:
    ...
    def calculate_gini_coefficient(self):
        ...
        n = len(wealth_values)
        gini = (np.sum((2 * np.arange(1, n + 1) - n - 1) * wealth_values)) / (n * np.sum(wealth_values))
        return gini
    ...
    def broadcast_messages(self):
        ...
        for i, agent in enumerate(self.agents):
            ...
            agent.broadcast_message(self, self.timestep, tree)
    ...
    def step(self):
        ...
        # Broadcast messages efficiently using cKDTree
    ...
    def run_simulation(self, max_timesteps=1000):
        while self.running and self.timestep < max_timesteps:
            ...
            self.step()
            self.render()
            self.clock.tick(10)  # Adjust simulation speed as needed
...
def main():
    ...
    env = SugarscapeEnvironment(width=50, height=50, cell_size=10,
                                params=rural_params, seed=seed)
